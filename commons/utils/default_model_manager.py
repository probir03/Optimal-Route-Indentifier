...
class DefaultManager(Manager):
    ...
    def get_paginated(self, filters=None, queries=None, projection=None,
        ...
        Returns:
            A list of model dictionaries matching the query and Integer for totla count and offset
        '''
        query = {}
        if queries:
            query = queries
        ...
        new_offset = (offset + limit) if (offset + limit) < count else count
        ...
        if limit:
            resources = resources.limit(limit)
        return list(resources.values()), count, new_offset
    ...
    def get_aggregated_mongo_result(self, group_by=None,  queries=None, sort=None, projection=None):
        '''Performs a mongo group by operation according to the value of group by passed.

        Args:
            group_by: field upon which group by is to be applied.
            projection: projection operator

        Returns:
            result(List): list of objects where each object represents a group by bucket.
            Ex : [ {
                '_id' : "",
                'values' : [{}]
            } ]

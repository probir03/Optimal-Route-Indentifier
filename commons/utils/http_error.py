...
class PymodmValidationError(BadRequest):
    '''Wrapper for Pymodm validation exception where it returns list including validation errors.

    It is a child class of BadRequest class
    '''
    status_code = 400

    def __init__(self, message='The request is invalid. Please try again.', error_code=None, errors=None):
        err_dict = eval(message)
        if isinstance(err_dict, dict):
            message = self.format_error(err_dict, '')[:-1]
        super(BadRequest, self).__init__(self.status_code, message, error_code, errors)

    def format_error(self, data=None, err_str=''):
        '''
        helper function for PymodmValidationError
        params:
            data: dict
            err_str: string
        '''
        for key, val in data.items():
            if isinstance(val, list) and val:
                if isinstance(val[0], str):
                    err_str = err_str + ' ' + str(key) + ': ' + str(val[0]) + ' &'
                else:
                    err_str = self.format_error(val[0], err_str)
        return err_str
